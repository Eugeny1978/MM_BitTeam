...
class BitTeam(): # Request
    ...
    def load_markets(self):
        ...
        Минимальный ОБЪЕМ  = limit_usd = эквивалентен указанной стоимости USD (фиатных долларов)
        ШАГ ОБЪЕМОВ = lot size tick / 10 ^(lot size view min) = (тик / на 10 в степени)
        ШАГ ЦЕН = price_tick / 10 ^ (price view min) = (тик / на 10 в степени)
        да, это лишние поля, но в теории они ПОЗВОЛЯЮТ сделать шаг не только 1 но и 2 и 5
        (остальные (3, 4, 6, 7, 8, 9) - не кратны 10 - поэтому некорректны)
        Например:
        шаг = 0.002
        шаг = 0.0005
        шаг = 2
